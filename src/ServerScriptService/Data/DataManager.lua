...
local formatString = require(game.ReplicatedStorage.Modules.Utils.FormatString)

function DataManager.UpdateSpeedrunTime(player: Player, speedrunName: string, timeSpent: number)
	local profile = DataManager.Profiles[player]
	if not profile then return end
	
	profile.Data.Speedruns[speedrunName] = timeSpent -- Update player data
end
...
function DataManager.AddTimePlayed(player: Player, timeAmount: number)
	...
	player.OtherValues.TimePlayed.Value += timeAmount
end

function DataManager.AddDeath(player: Player, deathAmount: number)
	local profile = DataManager.Profiles[player]
	if not profile then return end
	
	profile.Data.Deaths += deathAmount -- Update player data
	player.OtherValues.Deaths.Value += deathAmount
end
...
function DataManager.AddCoins(player: Player, coinsAmount: number)
	...
	player.leaderstats.Coins.Value += coinsAmount -- Update player UI InGame
...
function DataManager.AddKills(player: Player, killsAmount: number)
	local profile = DataManager.Profiles[player]
	if not profile then return end
	
	profile.Data.Kills += killsAmount -- Update player data
	player.leaderstats.Kills.Value += killsAmount
end

function DataManager.AddWins(player: Player, winsAmount: number)
	local profile = DataManager.Profiles[player]
	if not profile then return end
	
	profile.Data.Wins += winsAmount -- Update player data
	player.leaderstats.Wins.Value += winsAmount
end
...
function DataManager.AddFreeRevive(player: Player, reviveAmount: number)
	...
	player.OtherValues.FreeRevives.Value += reviveAmount
end
...
function DataManager.EquipChar(player: Player, newChar: string)
	...
	pcall(function()
		local equipedCharacter = player.OtherValues:WaitForChild("EquipedCharacter", 30)
		player.OtherValues.EquipedCharacter = newChar
	end)
end
...
function DataManager.UpdateSetting(player: Player, settingName: string, newValue)
	local profile = DataManager.Profiles[player]
	if not profile then return end
...
function DataManager.RemovePerk(player: Player, perkName: string, amount: number)
	local profile = DataManager.Profiles[player]
	if not profile then return end
	
	if profile.Data.OwnedPerks[perkName] then
		profile.Data.OwnedPerks[perkName] -= amount
		if profile.Data.OwnedPerks[perkName] <= 0 then
			profile.Data.OwnedPerks[perkName] = nil
			if profile.Data.EquipedPerks[perkName] then
				profile.Data.EquipedPerks[perkName] = nil
			end
		end
	end
