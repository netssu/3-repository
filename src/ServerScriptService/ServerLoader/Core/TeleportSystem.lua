...
function TeleportSystem.Init()
	...
	for i, teleporter in ipairs(Teleporters) do
		...
		local gameMode = Configuration:GetAttribute("GameMode")
		...
		local function TeleportsPlrs()
			...
			local TeleportData = {
				...
				["GameMode"] = gameMode or "Normal", -- if nil game mode is set to 'Normal'
			}
			...
			if placeID == "0" or placeID == "" or placeID == " " then
