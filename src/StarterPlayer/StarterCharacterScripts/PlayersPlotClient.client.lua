...
-- [[ THE FIX: Wait for the Server to assign the plot over the internet! ]]
if PlayerSpotVal.Value == "None" or PlayerSpotVal.Value == "" then
	PlayerSpotVal:GetPropertyChangedSignal("Value"):Wait()
end

local PlotIndicatorBilboard = game.ReplicatedStorage.UIAssets.PlotIndicator

-- Safely wait for the plot to exist before grabbing its Base
...
local RestaurantDetectionZone = PlayerPlot:WaitForChild("Base"):WaitForChild("DetectionZone")
...
local RestaurantZoneOParams = OverlapParams.new()
RestaurantZoneOParams.FilterType = Enum.RaycastFilterType.Include
RestaurantZoneOParams.FilterDescendantsInstances = {Character}
RestaurantZoneOParams.MaxParts = 1
...
local DEV_PRODUCTS = {
	...
	--GOURMET FOOD
	["GourmetFoodPerFood"] = 3562385273,
	["MultiplyGourmetFoodPerFood"] = 3562386003,
	["CashPerGourmetFood"] = 3562384451,
	["MultiplyCashPerGourmetFood"] = 3562386277,
	["CustomerRate"] = 3562384739,
	["GourmetCookingSpeed"] = 3562386881,
	["ChanceOfGoldenGourmet"] = 3562387147,
...
local function RestaurantAreaDetection(deltaTime)
	...
	if RestaurantSkillTree.UnlockRestaurant.Value == false then
		return
	end
	local partsFound = workspace:GetPartsInPart(RestaurantDetectionZone, RestaurantZoneOParams)
	
	-- PLAYER IS INSIDE ZONE
	if #partsFound >= RestaurantZoneOParams.MaxParts then
		for i,Seat in pairs(PlayerPlot:GetDescendants()) do
			if Seat:IsA("Seat") then
				Seat.CanTouch = false
			end
		end
		if InRestaurantZone == false then
			...
			MarketButtons.CashUpgradesButton.ImageColor3 = Color3.new(1, 0.917647, 0)
		...
		if InRestaurantZone == true then
			InRestaurantZone = false
			if RestaurantUpgradesUI.Visible == true then
				local Tween = TS:Create(RestaurantUpgradesUI,TweenInfo.new(0.35,Enum.EasingStyle.Back,Enum.EasingDirection.In,0,false,0),{Position = UDim2.fromScale(1.5,0.5)})
				Tween:Play()
				Tween.Completed:Once(function()
					RestaurantUpgradesUI.Visible = false
...
for i,BoardFrame in pairs(RestaurantUpgradesUI.ScrollingFrame:GetChildren()) do
...
for i,BTN in pairs(MarketButtons:GetDescendants()) do
	if BTN:IsA("GuiButton") then
		BTN.MouseButton1Click:Connect(function()
			if BTN.Name == "CashUpgradesButton" and InRestaurantZone == true then
				BTN.Visible = false
				RestaurantUpgradesUI.Position = UDim2.fromScale(1.5,0.5)
				RestaurantUpgradesUI.Visible = true
				TS:Create(RestaurantUpgradesUI,TweenInfo.new(0.6,Enum.EasingStyle.Elastic,Enum.EasingDirection.Out,0,false,0),{Position = UDim2.fromScale(0.685,0.5)}):Play()				
...
local ChefUnlocked = PlayerStats.RestaurantUnlocks.ChefUnlocked
local function ChefUnlockedTracking()
	if ChefUnlocked.Value == true then
		HUD.ManagerFrame.HireWorkers.ScrollingFrame.HireChef.Purchase.Title.Text = "UNLOCKED"
		HUD.ManagerFrame.HireWorkers.ScrollingFrame.HireChef.Purchase.RedGradient.Enabled = true
		HUD.ManagerFrame.HireWorkers.ScrollingFrame.HireChef.Purchase.GreenGradient.Enabled = false
		HUD.ManagerFrame.HireWorkers.ScrollingFrame.HireChef.Purchase.BackgroundColor3 = Color3.new(1, 1, 1)
		HUD.ManagerFrame.HireWorkers.ScrollingFrame.HireChef.Purchase.ButtonStroke.Color = Color3.new(0.560784, 0.0431373, 0.0235294)

	elseif ChefUnlocked.Value == false then
		HUD.ManagerFrame.HireWorkers.ScrollingFrame.HireChef.Purchase.Title.Text = "$250k"
		HUD.ManagerFrame.HireWorkers.ScrollingFrame.HireChef.Purchase.RedGradient.Enabled = false
		HUD.ManagerFrame.HireWorkers.ScrollingFrame.HireChef.Purchase.GreenGradient.Enabled = true
		HUD.ManagerFrame.HireWorkers.ScrollingFrame.HireChef.Purchase.BackgroundColor3 = Color3.new(1, 1, 1)
		HUD.ManagerFrame.HireWorkers.ScrollingFrame.HireChef.Purchase.ButtonStroke.Color = Color3.new(0.0980392, 0.313725, 0)
	end
end
ChefUnlockedTracking()
ChefUnlocked:GetPropertyChangedSignal("Value"):Connect(ChefUnlockedTracking)

------

local CustomizationUnlocked = PlayerStats.RestaurantUnlocks.CustomizationUnlocked
local function CustomizationUnlockedTracking()
	if CustomizationUnlocked.Value == true then
		HUD.ManagerFrame.CustomizeRestaurant.LockedFrame.Visible = false
	elseif CustomizationUnlocked.Value == false then
		HUD.ManagerFrame.CustomizeRestaurant.LockedFrame.Visible = true
	end
end
CustomizationUnlockedTracking()
CustomizationUnlocked:GetPropertyChangedSignal("Value"):Connect(CustomizationUnlockedTracking)

local RestaurantUnlocked = PlayerStats.RestaurantSkillTree.UnlockRestaurant
local function RestaurantUnlockedTracking()
	if RestaurantUnlocked.Value == true then
		HUD.StatsCorner.Stats["Gourmet Food"].Visible = true
	elseif RestaurantUnlocked.Value == false then
		HUD.StatsCorner.Stats["Gourmet Food"].Visible = false
	end
end
RestaurantUnlockedTracking()
RestaurantUnlocked:GetPropertyChangedSignal("Value"):Connect(RestaurantUnlockedTracking)
	
local ManagerFrame = HUD.ManagerFrame
local CustomizeRestaurantFrame = ManagerFrame.CustomizeRestaurant
for i,BTN in pairs(HUD.ManagerFrame:GetDescendants()) do
	if BTN:IsA("GuiButton") then
		BTN.MouseButton1Click:Connect(function()
			if BTN.Name == "Purchase" then
				local HireType = BTN.Parent.Name
				if CashVal.Value >= 250000 and HireType == "HireChef" then
					PlayerPlotRem:FireServer("PlayerHiring",HireType)
					print("Clicking: "..HireType)
				elseif CashVal.Value >= 75000 and HireType == "LockedFrame" and CustomizationUnlocked.Value == false then
					PlayerPlotRem:FireServer("UnlockCustomization")
					print("Clicking: "..HireType)
				end
			...
			local PartToBeColored = BTN.Parent.Parent.Parent.Parent
			local SectionOfPartToBeColored = BTN.Parent.Parent
			if PartToBeColored.Name == "BuildingColor" and SectionOfPartToBeColored:IsDescendantOf(PartToBeColored) and CustomizationUnlocked.Value == true then
				print(BTN.Name, PartToBeColored.Name,SectionOfPartToBeColored.Name)
				PlayerPlotRem:FireServer("ChangeBuildingColor",{BTN.Name,PartToBeColored.Name,SectionOfPartToBeColored.Name})
			elseif PartToBeColored.Name == "FurnitureColor" and SectionOfPartToBeColored:IsDescendantOf(PartToBeColored) and CustomizationUnlocked.Value == true then
				print(BTN.Name, PartToBeColored.Name,SectionOfPartToBeColored.Name)
				PlayerPlotRem:FireServer("ChangeFurnitureColor",{BTN.Name,PartToBeColored.Name,SectionOfPartToBeColored.Name})
			end
		end)
	end
end
...
for i,BTN in pairs(RestaurantUpgradesUI:GetDescendants()) do
	if BTN:IsA("GuiButton") then
		BTN.MouseButton1Click:Connect(function()
		if BTN.Name == "Buy" or BTN.Name == "Max" or BTN.Name == "RobuxMax" then
			...
			if BTN.Name == "Buy" then
				print("pressing buy on ".. UpgradeName)
			...
			if BTN.Name == "Close" then
				local Tween = TS:Create(RestaurantUpgradesUI,TweenInfo.new(0.5,Enum.EasingStyle.Back,Enum.EasingDirection.In,0,false,0),{Position = UDim2.fromScale(1.5,0.5)})
				Tween:Play()
				Tween.Completed:Once(function()
					RestaurantUpgradesUI.Visible = false
					MarketButtons.CashUpgradesButton.Visible = true
			...
			elseif BTN.Name == "Layout" then
				...
				if BTN:GetAttribute("CurrentLayout") == "Single" then
					TS:Create(RestaurantUpgradesUI.ScrollingFrame.UIGridLayout,TweenInfo.new(0.3,Enum.EasingStyle.Back,Enum.EasingDirection.Out,0,false,0),{CellPadding = UDim2.fromScale(0.05,0.003)}):Play()
					TS:Create(RestaurantUpgradesUI.ScrollingFrame.UIGridLayout,TweenInfo.new(0.35,Enum.EasingStyle.Back,Enum.EasingDirection.Out,0,false,0),{CellSize = UDim2.fromScale(0.43,0.062)}):Play()
					BTN.Image = "rbxassetid://138808955381977"
					BTN.Size = UDim2.fromScale(0.055,0.126)
					BTN.Position = UDim2.fromScale(0.44,0.875)
					RestaurantUpgradesUI.ScrollingFrame.ClipsDescendants = true
					RestaurantUpgradesUI.ScrollingFrame.CanvasPosition = Vector2.new(0,0)
					RestaurantUpgradesUI.ScrollingFrame.ScrollingEnabled = true
					RestaurantUpgradesUI.ScrollingFrame.ScrollBarImageTransparency = 0.25
					RestaurantUpgradesUI.ScrollArrow.Visible = true
					LayoutBtnBindable:Fire(BTN)
					BTN:SetAttribute("CurrentLayout","Multi")
					
				elseif BTN:GetAttribute("CurrentLayout") == "Multi" then
					TS:Create(RestaurantUpgradesUI.ScrollingFrame.UIGridLayout,TweenInfo.new(0.3,Enum.EasingStyle.Back,Enum.EasingDirection.Out,0,false,0),{CellPadding = UDim2.fromScale(0.05,0.01)}):Play()
					TS:Create(RestaurantUpgradesUI.ScrollingFrame.UIGridLayout,TweenInfo.new(0.35,Enum.EasingStyle.Back,Enum.EasingDirection.Out,0,false,0),{CellSize = UDim2.fromScale(0.9,0.127)}):Play()
			...
			elseif BTN.Name == "ScrollArrow" then
				...
				local ScrollingFrame: ScrollingFrame = RestaurantUpgradesUI.ScrollingFrame
	...
	if BTN:IsA("GuiButton") then
		BTN.MouseButton1Click:Connect(function()
			if BTN.Name == "100" or BTN.Name == "50" or BTN.Name == "25" then
				PlayerPlotRem:FireServer("RestockStorageBox",{Percent = BTN.Name,InRestaurant = InRestaurantZone})
			...
			elseif BTN.Name == "PutInFoodStat" then
				PlayerPlotRem:FireServer("GivePlayerFoodFromFoodBoxes",InRestaurantZone)
			elseif BTN.Name == "Close" then
...
for i,CookGourmetPrompt in pairs(CS:GetTagged("CookGourmetFoodPrompt")) do
	if CookGourmetPrompt:IsA("ProximityPrompt") then
		CookGourmetPrompt.PromptShown:Connect(function()
			local Plot = CookGourmetPrompt.Parent.Parent.Parent.Parent.Parent
			local Occupant = Plot.Info.Occupant
			if Occupant.Value ~= Player.Name then
				return
			end
			local Stove = Plot.RestaurantBuild.Kitchen.Stove
			local Ports = Stove.Ports
			local NextFreePort = nil
			for i = 1,12 do
				if Ports:FindFirstChild("PortFrame"..i).InUse.Value == false then
					NextFreePort = Ports:FindFirstChild("PortFrame"..i)
					break
				end
			end
			if NextFreePort == nil then
				if PlayerGui:FindFirstChild("ProximityPrompts") and PlayerGui:FindFirstChild("ProximityPrompts"):FindFirstChild("Default") then
					PlayerGui:FindFirstChild("ProximityPrompts"):WaitForChild("Default").Enabled = false
				end
				return
			end
			if Stove:FindFirstChild("StoveHighlight")  then
				local StoveHighlight:Highlight = Stove:FindFirstChild("StoveHighlight")
				StoveHighlight.Parent = NextFreePort
				StoveHighlight.Enabled = true	
			end
			if Stove:FindFirstChild("NextPotDisplay") then
				local NextPotDisplay:BillboardGui = Stove:FindFirstChild("NextPotDisplay")
				NextPotDisplay.Parent = NextFreePort
				NextPotDisplay.Enabled = true	
			end
			
			print("Player Prompt Shown: "..Plot.Name)
		end)	
		
		CookGourmetPrompt.PromptHidden:Connect(function()
			local Plot = CookGourmetPrompt.Parent.Parent.Parent.Parent.Parent
			local Occupant = Plot.Info.Occupant
			if Occupant.Value ~= Player.Name then
				return
...
local function SetupPlotIndicators()
	for _, Plot in pairs(Plots:GetChildren()) do
		...
		local OccupantVal = Plot:FindFirstChild("Info") and Plot.Info:FindFirstChild("Occupant")
		if not OccupantVal or OccupantVal.Value == "" or OccupantVal.Value == "None" then continue end

		local OccupantName = OccupantVal.Value
		local OccupantPlayer = Players:FindFirstChild(OccupantName)
		local OccupantUserId = OccupantPlayer and OccupantPlayer.UserId

		if not OccupantUserId then continue end

		local IndicatorClone = PlotIndicatorBilboard:Clone()
		IndicatorClone.Parent = BasePart

		local Frame = IndicatorClone:FindFirstChild("Frame")
		if not Frame then continue end

		if OccupantName == Player.Name then
			Frame.PlayerName.Text = "HOME"
		else
			Frame.PlayerName.Text = OccupantName .. "'s PLOT"
		end
		...
		-- 🔴 NOVO: Remove o indicator quando o player sair e o plot ficar vazio
		Players.PlayerRemoving:Connect(function(LeavingPlayer)
			if LeavingPlayer.Name ~= OccupantName then return end

			-- Aguarda um frame para o servidor atualizar o OccupantVal
			task.wait(0.5)

			if OccupantVal.Value == "" or OccupantVal.Value == "None" or OccupantVal.Value ~= LeavingPlayer.Name then
				if IndicatorClone and IndicatorClone.Parent then
					IndicatorClone:Destroy()
				end
			end
		end)

		-- 🔴 NOVO: Também detecta se o Occupant mudar pelo servidor (ex: plot liberado)
		OccupantVal:GetPropertyChangedSignal("Value"):Connect(function()
			if OccupantVal.Value == "" or OccupantVal.Value == "None" then
				if IndicatorClone and IndicatorClone.Parent then
					IndicatorClone:Destroy()
				end
			end
...
-- Roda em Task.spawn pois GetUserThumbnailAsync é yielding
task.spawn(SetupPlotIndicators)

RunService.Heartbeat:Connect(function(deltaTime)
	RestaurantAreaDetection(deltaTime)
	--
end)

for i,ClickDetector in pairs(PlayerPlot:GetDescendants()) do
	if ClickDetector:IsA("ClickDetector") then
		
	end
end

PlayerPlotRem.OnClientEvent:Connect(function(Action,Data)
	...
	elseif Action == "OpenManagerFrame" then
		local ManagerFrame = HUD.ManagerFrame
		ManagerFrame.Visible = true
		ManagerFrame.Position = UDim2.fromScale(1.5,0.5)
		TS:Create(ManagerFrame,TweenInfo.new(0.65,Enum.EasingStyle.Elastic,Enum.EasingDirection.Out,0,false,0),{Position = UDim2.fromScale(0.5,0.5)}):Play()
	...
	elseif Action == "PotCooked" and Data["Pot"] then
		local Pot:Model = Data["Pot"]
	...
	elseif Action == "MoneySplash" and Data then
		...
		for i,v in pairs(MoneySplashVFX:GetChildren()) do
			if v:IsA("ParticleEmitter") then
				v.Lifetime = NumberRange.new(2.5)
