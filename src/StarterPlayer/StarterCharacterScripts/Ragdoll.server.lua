...
local ColliderList = {};
	...
	task.wait(0.1) -- Delay to player humanoid be able to fire Hum.Died event.
	...
	if (Humanoid.Health <= 0) then
		...
		RagDollModel.Name = Character.Name.."_RagDoll"
...
--game.Debris:AddItem(RagDollModel, 30);
	...
	if RagDollModel then
		...
		local face_blink = RagDollModel.Head:FindFirstChild("face_Blink")
		local face_SemiBlink = RagDollModel.Head:FindFirstChild("face_SemiBlink")
		if face and face ~= nil then
			face.Transparency = 1
			face_blink.Transparency = 0
			face_SemiBlink.Transparency = 1
		end
