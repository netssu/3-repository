...
function CustomPrompts.Init()
	...
	local function createPrompt(prompt, inputType, gui)
		...
		local backgroundTransparencyButton = button.BackgroundTransparency
		local uiStrokeTransparency = uiStroke.Transparency
		button.BackgroundTransparency = 1
		button.Transparency = 1

		table.insert(tweensForFadeIn, TweenService:Create(button, tweenInfoFast, {BackgroundTransparency = backgroundTransparencyButton}))
		table.insert(tweensForFadeIn, TweenService:Create(uiStroke, tweenInfoFast, {Transparency = uiStrokeTransparency}))
		table.insert(tweensForFadeOut, TweenService:Create(button, tweenInfoFast, {BackgroundTransparency = 1}))
		...
		if prompt.HoldDuration > 0 then
			...
			local normalBar = resizeableInputFrame.ProgressBar2.Bar
			setUpCircularProgressBar(circleBar)
			table.insert(tweensForButtonHoldBegin, TweenService:Create(normalBar, tweenInfoInFullDuration, { BackgroundTransparency = 0.5, Size = UDim2.fromScale(1, 1) }))
			...
			table.insert(tweensForButtonHoldEnd, TweenService:Create(normalBar, tweenInfoInstant, { BackgroundTransparency = 1, Size = UDim2.fromScale(1, 0)}))
		end
