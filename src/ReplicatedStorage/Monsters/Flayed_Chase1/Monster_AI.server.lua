...
--//Anims
local PushAnim = script.Parent.Animator.Animations:FindFirstChild("Push")
...
local function Jumpscare(target: Player)
	if plrsJumpscare[target] then return end
	plrsJumpscare[target] = true
...
task.wait(1.8) -- delay before start chasing

for _, spot in MonsterWaypoints:GetChildren() do
