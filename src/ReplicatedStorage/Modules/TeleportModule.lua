...
local ZoneTransitionEvent = Rs:FindFirstChild("Remotes"):FindFirstChild("ZoneTransition")

--//Values
local MaxWaitTime = 15

-- Teleport alive player to a determined game zone.
		...
		for i, plr in ipairs(Players:GetPlayers()) do
			...
			if HumanoidRootPart and IsAlive.Value then -- If not IsAlive value is because player is specting.
				...
				if not QuickTeleport then
					plrTransition = ZoneTransitionEvent:InvokeClient(plr, zoneTeleport.Name)
				end
				...
				repeat wait()
				until plrTransition == true or maxTimeReached == true
