...
local function createPrompt(prompt, inputType, gui)
	...
	local tweenInfoSlowElastic = TweenInfo.new(0.6, Enum.EasingStyle.Elastic, Enum.EasingDirection.Out)
	...
	table.insert(tweensForButtonHoldEnd, TweenService:Create(frame, tweenInfoSlowElastic, { Size = UDim2.fromScale(1, 1), BackgroundTransparency = backgroundTransparency, ImageTransparency = imageTransparency }))
	...
	local function setupUIStrokeTweens(uiStroke)
		...
		table.insert(tweensForButtonHoldEnd, TweenService:Create(uiStroke, tweenInfoSlowElastic, { Transparency = transparency }))
	...
	local function setupGUIObjectTweens(guiObject)
		...
		table.insert(tweensForButtonHoldEnd, TweenService:Create(guiObject, tweenInfoSlowElastic, { BackgroundTransparency = guiObjectBackgroundTransparency }))
	...
	local function setupTextLabelTweens(textLabel)
		...
		table.insert(tweensForButtonHoldEnd, TweenService:Create(textLabel, tweenInfoSlowElastic, { TextTransparency = textTransparency, TextStrokeTransparency = textStrokeTransparency }))
	...
	local function setupImageLabelTweens(imageLabel)
		...
		table.insert(tweensForButtonHoldEnd, TweenService:Create(imageLabel, tweenInfoSlowElastic, { ImageTransparency = imageTransparency }))
	...
	table.insert(tweensForButtonHoldBegin,TweenService:Create(inputFrame,tweenInfoOutHalfSecond,{ Position = UDim2.fromScale(0.185,0)}))
	...
	table.insert(tweensForButtonHoldEnd,TweenService:Create(inputFrame,tweenInfoFast,{ Position = UDim2.fromScale(0,0)}))
	...
	local function updateUIFromPrompt()
		...
		local PromptsParent = prompt.Parent
		if not prompt.Parent:IsA("Model") then
			PromptsParent = prompt.Parent.Parent
		end
		print(PromptsParent.Name)
		if PromptsParent:GetAttribute("NPCType") == "Worker" then 
			if actionText:FindFirstChild("CommonGradient") then
				return
			end
			local CashGradient = game.ReplicatedStorage:WaitForChild("UIAssets").CommonGradient:Clone()
			CashGradient.Parent = actionText
		end
