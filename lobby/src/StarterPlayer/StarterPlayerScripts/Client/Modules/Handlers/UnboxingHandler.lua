...
local function Unbox(UnitName: string, CrateName: string)
	...
	local success, err = pcall(function()
		...
		if not SelectedCrate then
			warn("[UnboxingHandler] Missing crate model:", CrateName, "using Normal model as fallback")
			SelectedCrate = BoxesFolder:FindFirstChild("Normal")
		end
		if not SelectedCrate then error("Crate missing: " .. tostring(CrateName)) end
